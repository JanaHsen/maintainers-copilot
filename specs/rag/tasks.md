...
- [ ] T028 [US3] [P] Implement `evals/rag/score.py`: pure retrieval helpers `recall_at_k`, `mrr`, `ndcg` (no I/O — takes `predictions` + `golden` as in-memory lists). **Also** add `build_ragas_dataset(rows: list[dict]) -> ragas.EvaluationDataset` that converts `(question, retrieved_contexts, response, reference)` tuples into the [`ragas.EvaluationDataset`](https://docs.ragas.io/en/stable/concepts/components/eval_dataset/) format consumed by `ragas.evaluate()` in T036. Acceptance: unit tests cover the three retrieval metrics against hand-rolled inputs with known expected values; a fourth test feeds two example rows through `build_ragas_dataset` and asserts the resulting object has the documented columns. Rules: 5, 9.
...
- [ ] T035 [US3] Wire the **RAGAS judge** in place of a hand-rolled prompt: (a) add `ragas>=0.2` and `langchain-anthropic>=0.2` to the `ml` dependency group in `pyproject.toml`; (b) add `evals/rag/ragas_judge.py` that configures the RAGAS judge LLM as Claude Haiku via `langchain_anthropic.ChatAnthropic` (`model="claude-haiku-4-5-20251001"`, temperature 0, API key from Vault via `app.infra.anthropic_client`) and the RAGAS embeddings as a `sentence-transformers` wrapper over `BAAI/bge-base-en-v1.5` (already cached in the model server image — the wrapper instantiates the model in-process, no network call); (c) add `prompts/rag_answer.md` — a short two-section system+user generation prompt the eval uses to produce an answer from the retrieved chunks **before** RAGAS judges it. Commit message: `wire RAGAS judge (Claude Haiku) + sentence-transformers embeddings + generation prompt`. Acceptance: `uv lock` resolves cleanly with the new deps; `python -c "from evals.rag.ragas_judge import build_judge; j, e = build_judge(); print(type(j).__name__, type(e).__name__)"` prints `ChatAnthropic` + the ST-wrapper class name; `prompts/rag_answer.md` parses via `model_server/prompts.py::load_system_user`. Rules: 6, 9.

- [ ] T036 [US3] Extend `evals/rag/eval_rag.py` with the **generation eval via RAGAS**: for each golden question, (a) call `/retrieve` to get the top-5 chunks, (b) generate an answer with Claude Haiku using `prompts/rag_answer.md` (via `app/infra/anthropic_client.complete`), (c) build a RAGAS sample via `score.py::build_ragas_dataset` (T028), (d) call `ragas.evaluate(dataset, metrics=[LLMContextRecall(), Faithfulness(), ResponseRelevancy()], llm=<judge>, embeddings=<judge embeddings>)` with the judge configured in T035, (e) aggregate to mean scores. Write the results under the report's `generation` block with the exact keys `faithfulness`, `answer_relevancy`, `context_recall`. Commit message: `wire RAG generation eval via RAGAS (Claude Haiku judge + ST embeddings)`. Acceptance: a dry run with `--mode advanced --max-questions 3` prints the three generation scores; the report JSON's `generation` block has exactly those three keys with float values in [0, 1]. Rules: 2, 5, 6, 7, 11.
