...
- Generation judge: frozen Claude Haiku with versioned prompt via existing `app/infra/anthropic_client.py` — committed in T035.
...
- [ ] T028 [US3] [P] Implement `evals/rag/score.py`: pure retrieval helpers `recall_at_k`, `mrr`, `ndcg` (no I/O — takes `predictions` + `golden` as in-memory lists). No RAGAS dependency — the generation eval in T036 calls Claude Haiku directly via the existing `app/infra/anthropic_client.py`. Acceptance: unit tests cover the three retrieval metrics against hand-rolled inputs with known expected values. Rules: 5, 9.

- [ ] T029 [US3] Implement `evals/rag/eval_rag.py` with two **modes** selected by `--mode {naive,advanced}`. **Naive mode**: queries `rag_chunks` directly via `chunk_repository.query_first_stage` with `alpha=1.0`, fixed-400-char child chunks only, no rerank, no HyDE, k=5, no metadata filter; computes the retrieval metrics over the golden set. **Advanced mode**: routes through the live `/retrieve` endpoint (this is what CI runs). Both modes write the report under `evals/reports/{run_ts}/rag.json` with the **exact top-level keys** `retrieval: { hit_at_5, mrr_at_10, ndcg }` and `generation: { faithfulness, answer_relevancy, context_recall }` (generation block is empty `{}` at this point — populated in T036). Both `--upload-report` to MinIO. Acceptance: `uv run python evals/rag/eval_rag.py --mode naive --skip-upload` against the smoke corpus prints a metrics block under the documented top-level keys; the same command in advanced mode against the live api prints metrics + writes the report. Rules: 5, 9, 10.
...
- [ ] T035 [US3] Wire a **frozen Claude Haiku judge** via the existing `app/infra/anthropic_client.py` — no new deps. (a) Add `prompts/rag_answer.md`: a short two-section system+user generation prompt the eval uses to produce an answer from the retrieved chunks **before** the judge scores it. (b) Add `prompts/rag_judge.md`: a versioned two-section system+user prompt that scores each `(question, retrieved_contexts, generated_answer)` tuple on three axes — `faithfulness` (0–1), `answer_relevancy` (0–1), `context_recall` (0–1). The prompt MUST enforce JSON-only output (system block instructs the model to return exactly `{"faithfulness": <float>, "answer_relevancy": <float>, "context_recall": <float>}` with no prose, no markdown fences). Both prompts include an explicit version stamp in the header so future revisions are auditable. (c) No edits to `pyproject.toml` — no `ragas`, no `langchain-anthropic`. Commit message: `add frozen Claude Haiku judge + answer-generation prompts (versioned, JSON-only)`. Acceptance: `prompts/rag_answer.md` and `prompts/rag_judge.md` both parse via `model_server/prompts.py::load_system_user`; the judge prompt's system block contains a literal "JSON" instruction and an explicit version string; `pyproject.toml` is unchanged compared to T034. Rules: 6, 9.

- [ ] T036 [US3] Extend `evals/rag/eval_rag.py` with the **generation eval via the frozen Claude Haiku judge**: for each golden question, (a) call `/retrieve` to get the top-5 chunks, (b) generate an answer with Claude Haiku using `prompts/rag_answer.md` (via `app/infra/anthropic_client.complete`), (c) call Claude Haiku a second time using `prompts/rag_judge.md` to score the `(question, retrieved_contexts, generated_answer)` tuple, (d) parse the model's JSON-only response into `{faithfulness, answer_relevancy, context_recall}` floats, (e) aggregate to mean scores across the golden set. Write the results under the report's `generation` block with the exact keys `faithfulness`, `answer_relevancy`, `context_recall` — same key names as the brief's template, just produced without RAGAS. JSON-parse failures count as a hard error for that row (the row is logged and skipped; aggregation reports the count of skipped rows alongside the means). Commit message: `wire RAG generation eval via frozen Claude Haiku judge (no RAGAS)`. Acceptance: a dry run with `--mode advanced --max-questions 3` prints the three generation scores; the report JSON's `generation` block has exactly those three keys with float values in [0, 1]; no `ragas` or `langchain-anthropic` import appears anywhere under `evals/rag/`. Rules: 2, 5, 6, 7, 11.

- [ ] T037 [US3] **STOP AND ASK**: With the advanced numbers visible from T031–T036, fill `eval_thresholds.yaml`'s `rag:` section per user input #5 — `rag.hit_at_5_floor` and `rag.mrr_at_10_floor` (key names mirror the report's `retrieval` block from T029) set to a 5-point buffer below the observed advanced numbers; include a `notes:` field explaining the buffer choice (mirror the existing `classifier.notes`). Acceptance: the YAML parses; values are non-zero (Rule 4 — no `enforced: true` with floor 0); the comment block above the values cites the source advanced report. Rules: 4, 5, 6.
