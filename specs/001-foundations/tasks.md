...
- [ ] T028 [US2] Create `scripts/dataset/fetch_issues.py`: GitHub REST closed issues for `scikit-learn/scikit-learn`, PAT from Vault, concurrency-capped/rate-limit aware, writes verbatim JSONL to MinIO `raw/scikit-learn/issues/{run_id}/page_{n}.jsonl`; never overwrites a prior `run_id`. Acceptance: `uv run python scripts/dataset/fetch_issues.py` then MinIO console shows `raw/scikit-learn/issues/<run_id>/page_1.jsonl`; a second run creates a new `run_id` leaving the first byte-unchanged. Rules: 2, 3, 9.
- [ ] T027a [US2] After T028 has produced raw JSONL, create `scripts/dataset/inventory_labels.py` that reads every issue's `labels` from `raw/scikit-learn/issues/{run_id}/page_*.jsonl` in MinIO, computes a `Counter` of unique label names, and writes `scripts/dataset/observed_labels.txt` (one line per label: `<count>\t<label>`, sorted descending). Acceptance: `uv run python scripts/dataset/inventory_labels.py --run-id <id>` produces `observed_labels.txt` listing every unique scikit-learn label and its frequency. Rules: 6, 9.

- [X] T027b [US2] Commit `scripts/dataset/label_map.yaml` grounded in `observed_labels.txt`: precedence list `[bug, feature, docs, question]`, per-class lists drawn from the actual top-frequency scikit-learn labels, `drop_if_unmapped: true`. Add a `DECISIONS.md` entry citing the per-label counts used. Acceptance: `uv run python -c "import yaml; d=yaml.safe_load(open('scripts/dataset/label_map.yaml')); assert set(d['classes'])=={'bug','feature','docs','question'}"` exits 0; every label in `label_map.yaml` appears in `observed_labels.txt`. Rules: 6, 9.

- [ ] T029 [US2] Create `scripts/dataset/build_splits.py`: read raw JSONL, apply `label_map.yaml`, drop unmappable, stratify by class, time-sort so test = most recent 15% (train/val ≈70/15), write `train/val/test.parquet` + `splits_report.json` under `processed/scikit-learn/{run_id}/`. Acceptance: `uv run python scripts/dataset/build_splits.py` produces the four objects in MinIO; `splits_report.json` shows `test_min_closed_at > train_val_max_closed_at`, only the four classes present, and counts summing to `total_mapped` (contracts C2–C5). Rules: 3, 6, 9.
...
- [ ] T037 Run the dataset pipeline end-to-end and confirm **`splits_report.json` is visible in MinIO** under `processed/scikit-learn/{run_id}/`, satisfying contracts C2–C5. Acceptance: MinIO console screenshot/object path recorded; counts sum to `total_mapped` and test is strictly newer. Rules: 3, 6.
